 ...
 *
 * Now that we know how to query all records from a table utilizing the "SELECT"
 * keyword, it might be beneficial to filter what records are given to us from a
 * table.
 *
 ...
 *
 * Let's say we wanted to query all the records from the table that have the
 * first name "Steve".
 *
 * The statement that will be utilized is as follows:
 * SELECT * FROM employee WHERE first_name = 'Steve';
 *
 * In the above statement, the "WHERE" keyword allows us to filter the records
 * based on the condition defined after the keyword.
 * The condition that we specified is that the first_name column must have the
 * value "Steve".
 *
 ...
 * |3 |'Steve' |'Jones' |99890.99 |
 *
 * In addition to filtering on equality like above, we can filter on inequality
 * with the <, >, <=, >=, and != operators.
 * We can even filter on strings that match partially, using the LIKE keyword
 * and the '%' wildcard. Here are a few examples:
...
public class FilterRecords {
    ...
    /**
     * Problem 1: Given the employee table, write a query in the problem1.sql file
    ...
    /**
     * Problem 2: Given the employee table, write a query in the problem2.sql file
     * to retrieve all the records

 ...
 *
 * As of right now, all the data that we are storing into variables in java are
 * lost when the application ends.
 ...
 * - constraint:
 * - a constraint is a rule we can add to a column that the data we insert must
 * follow.
 * - Common Constraints:
 ...
 * - If a constraint that was defined is not met when inserting a record, a
 * SQLException will be thrown.
 */
public class CreateATable {

    /**
     * Assignment: Create a new table in the problem1.sql file, called "song" with 2
     * columns "title" and "artist".
     * both columns should have the datatype varchar(100), which represents a String

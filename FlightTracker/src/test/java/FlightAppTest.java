...
/**
 * This class tests using the dummy data inserted as part of the
 * Application.Application.databaseSetup() method.
...
public class FlightAppTest {
    ...
    /**
     * The flightDAO should retrieve a flight with a specific ID when getFlightById
     * is called.
     */
    @Test
    ...
    /**
     * When there is one flight between two cities, getAllFlightsFromCityToCity
    ...
    /**
     * When there are multiple flights between two cities,
     * getAllFlightsFromCityToCity should return a list containing
    ...
    /**
     * When a flight is added via the flightDAO, it should be retrievable by
     * retrieving the flight by ID.
     */
    @Test
    ...
    /**
     * When a flight is updated via the flightDAO, the updated values should be
     * retrieved when the flight is next
     * accessed.
     */
    @Test
    ...
    /**
     * when a flightDAO returns all flights, flightService.getAllFlights should
     * return all flights.
     */
    @Test
    ...
    /**
     * When a flightAdded is called and the database does not already contain the
     * flight,
     * the flight should be returned because the add should be successful.
     * Subsequent attempts to add the flight
     * should return null as the flight already exists. Also, verify that the
     * addFlight method of the flightDAO is
     * actually called.
     */
    @Test
    public void flightService_AddFlightTest() {
        ...
        // verify that addFlight was actually used (Mockito.any will accept any
        // parameters)
        Mockito.verify(mockFlightDAO).insertFlight(Mockito.any());
    }

    /**
     * Regardless of if the flights are filtered with a SQL query or in the Java
     * service class,
     * flightService.getAllFlightsFromCityToCity should return all the flights
     * between two cities.
     */
    @Test
    public void flightService_GetFlightsFromCityToCityTest1() {
        ...
        // both getAllFlights and getAllFlightsFromCityToCity are mocked in case you
        // want to develop a solution
        // that relies on filtering within the Service rather than in the SQL query.
        // Filtering in Java
    ...
    /**
     * When a flight exists, attempting to update it should return the updated
     * flight. Also, verify that the
    ...
    /**
     * When a flight does not exist, attempting to update it should return null.
     * Also, verify that

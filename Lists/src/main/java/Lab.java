...
/**
 * Lists are ordered collections. This means that we could access elements at a
 * certain position of a list,
 * and lists are also allowed to contain duplicate values.
 *
 * We will be using lists quite a bit to store values. They will be the most
 * common Collection we use, and you will
...
public class Lab {
     ...
     * Two interesting things here: First, List is actually an interface, not a
     * class: its only ability is to define
     * potential behavior, but it does not provide implementation code.
     *
     * Second, the <Integer> portion represents a Generic. Generics allow an object
     * to contain any object of a certain
     * type, so we can create a List that contains Integers, Strings, or even other
     * Lists. Why do we use Integer and
     * not int? Generics can only contain Objects, and not primitives, so we Integer
     * because it is a Wrapper class,
     * the Object form of a primitive.
     *
     * A popular implementation of List is ArrayList. Look up how to instantiate
     * one.
     *
     ...
     * @return nothing, pass by reference will cause changes to the list object to
     *         be reflected across the program.
     */
     ...
     * Lists, like arrays, are zero-indexed, so they start counting at zero. For
     * instance,
     ...
     * We can remove an item from the list, which will cause all items after it to
     * reduce their index by 1 (they are
     ...
     * We can update a value in the list, which will overwrite a value at a certain
     * position.
     *
     * @param list     a List<Integer> object that we would like to modify.
     * @param position represents the index of the element we would like to change.
     * @param value    the new value which we would like to assign to the item at
     *                 position in list
     * @return nothing, pass by reference will cause changes to the list object to
     *         be reflected across the program.
     */
